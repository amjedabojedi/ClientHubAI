...
You can download a copy if you have it saved locally and re-upload it.`;
  ...
  // ===== PAYMENT RECONCILIATION (Phase 1: read-only) =====
  // Aggregates existing billing data into a reconciliation view: billed vs
  // collected vs outstanding, computed overall + per-therapist + per-session.
  // Reuses the exact same role scoping as /api/billing/reports. No writes,
  // no new tables. Client names are reduced to initials for the accountant role.
  app.get("/api/reconciliation/summary", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      if (!req.user) return res.status(401).json({ message: "Authentication required" });

      // Reconciliation is a finance/management overview: restrict to the same
      // roles the frontend nav/route allows. Defense-in-depth: never rely on the
      // UI gate alone.
      const role = (req.user.role || '').toLowerCase().trim();
      if (!['admin', 'administrator', 'supervisor', 'accountant'].includes(role)) {
        return res.status(403).json({ message: "Access denied" });
      }

      const { startDate, endDate, therapistId } = req.query;

      // Validate the optional therapist filter up front (reject garbage ids).
      let filterTherapistId: number | undefined;
      if (therapistId && therapistId !== 'all') {
        filterTherapistId = parseInt(therapistId as string);
        if (Number.isNaN(filterTherapistId)) {
      ...
      if (role === "supervisor") {
        ...
        if (assignments.length === 0) {
          return res.json({ summary: emptySummary, therapists: [], rows: [] });
        }
        supervisedTherapistIds = assignments.map(a => a.therapistId);
        if (filterTherapistId !== undefined && supervisedTherapistIds.includes(filterTherapistId)) {
          supervisedTherapistIds = [filterTherapistId];
        }
      } else {
        // admin / administrator / accountant: optional UI filter
        resolvedTherapistId = filterTherapistId;
      }
      ...
      const toCents = (v: any) => Math.round((parseFloat(v ?? '0') || 0) * 100);
      const rate = (num: number, den: number) => den > 0 ? Math.round((num / den) * 1000) / 10 : 0;

      let billedC = 0, collectedC = 0, outstandingC = 0;
      const statusCounts: Record<string, number> = {};
      const byTherapist = new Map<number, { therapistId: number; therapistName: string; sessionCount: number; billedC: number; collectedC: number; outstandingC: number }>();
      const rows: any[] = [];

      for (const r of reports as any[]) {
        const b = r.billing || {};
        const rowBilled = toCents(b.totalAmount);
        const rowCollected = toCents(b.clientPaidAmount) + toCents(b.insurancePaidAmount);
        const rowOutstanding = Math.max(rowBilled - rowCollected, 0);
        billedC += rowBilled;
        collectedC += rowCollected;
        outstandingC += rowOutstanding;

        const status = b.paymentStatus || 'pending';
        statusCounts[status] = (statusCounts[status] || 0) + 1;

        const tid = r.therapist?.id;
        const tname = r.therapist?.fullName || 'Unknown';
        if (tid != null) {
          if (!byTherapist.has(tid)) {
            byTherapist.set(tid, { therapistId: tid, therapistName: tname, sessionCount: 0, billedC: 0, collectedC: 0, outstandingC: 0 });
          }
          const t = byTherapist.get(tid)!;
          t.sessionCount += 1;
          t.billedC += rowBilled;
          t.collectedC += rowCollected;
          t.outstandingC += rowOutstanding;
        }

        const fullName = r.client?.fullName || '';
        rows.push({
          id: b.id,
          sessionId: r.session?.id,
          sessionDate: r.session?.sessionDate,
          clientName: isAccountant ? clientInitials(fullName) : fullName,
          therapistId: tid,
          therapistName: tname,
          serviceCode: r.service?.serviceCode || b.serviceCode,
          serviceName: r.service?.serviceName || '',
          billed: rowBilled / 100,
          collected: rowCollected / 100,
          outstanding: rowOutstanding / 100,
      ...
      const therapists = Array.from(byTherapist.values())
        .map(t => ({
          ...
          billed: t.billedC / 100,
          collected: t.collectedC / 100,
          outstanding: t.outstandingC / 100,
          collectionRate: rate(t.collectedC, t.billedC),
        }))
        .sort((a, b) => b.outstanding - a.outstanding);

      const summary = {
        sessionCount: reports.length,
        totalBilled: billedC / 100,
        totalCollected: collectedC / 100,
        totalOutstanding: outstandingC / 100,
        collectionRate: rate(collectedC, billedC),
        statusCounts,
      };

      res.json({ summary, therapists, rows });
    } catch (error) {
