...
(async () => {
  try {
    ...
    try {
      ...
      log(
        "Server will start anyway. Database operations may fail until connection is established.",
      );
    ...
    try {
      ...
      setInterval(async () => {
        try {
          ...
          console.error(
            "[CRON] Error processing scheduled notifications:",
            error,
          );
        }
    ...
    app.use((err: any, req: Request, res: Response, next: NextFunction) => {
      ...
      log(
        `[ERROR] ${timestamp} - ${req.method} ${req.path} - Status: ${status} - ${message}`,
      );
      if (err.stack) {
