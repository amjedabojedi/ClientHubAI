...
export class NotificationService {
  ...
  /**
   * Sends password reset email with domain-aware reset link
   */
  async sendPasswordResetEmail(email: string, token: string, username: string): Promise<void> {
    try {
      // Determine the correct domain based on environment
      const baseUrl = process.env.NODE_ENV === 'production' 
        ? 'https://resiliencecrm.replit.app' 
        : 'http://localhost:5000';
      ...
      const emailContent = {
        content: {
          from: {
            name: process.env.FROM_NAME || 'TherapyFlow',
            email: process.env.FROM_EMAIL || 'noreply@therapyflow.com'
          },
          subject: 'Reset Your Password - TherapyFlow',
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #333;">Reset Your Password</h2>
              <p>Hello ${username},</p>
              <p>We received a request to reset your password for your TherapyFlow account.</p>
              ...
              <p style="color: #666; font-size: 12px;">TherapyFlow - Practice Management System</p>
            </div>
          `,
          text: `Hello ${username},
...
TherapyFlow - Practice Management System`
        },
        recipients: [{ address: email }]
      };

      await sp.transmissions.send(emailContent);
