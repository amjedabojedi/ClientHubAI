 ...
 * mini-form can RECOVER from a concurrency conflict in place: after the server
 * rejects a stale payment with HTTP 409 / STALE_PAYMENT_STATE, the amber
 * conflict banner appears, clicking "Reload latest totals" pulls the new
 * already-paid figures into the open mini-form, and the follow-up submit
 * succeeds against the now-fresh basis.
 *
 ...
 * lose what they typed). The client-profile Billing tab mini-form
 * (client/src/pages/client-detail.tsx) is a SEPARATE surface from the Billing
 * Dashboard PaymentDialog, with its OWN mutation (updatePaymentStatusMutation)
 * and its own refreshLatestPaymentTotals handler. The sibling suite
 * test/client-detail-payment-stale-message-ui.test.ts only locks in that the
 * 409 produces a specific message; it does NOT exercise the reload button or
 * prove the re-submit then succeeds. A regression in refreshLatestPaymentTotals
 * (e.g. it stopped updating the form's basis, or the re-submit kept carrying the
 * stale expectedPreviousForSource) would silently push staff back to the manual
 * close/reopen experience — or worse, let a re-submit run against a stale basis.
 * This suite covers the full recovery path for the second surface.
 *
 ...
 *      0 was paid, so its submit carries expectedPreviousForSource = 0 against an
 *      authoritative 30 → the exact stale condition.
 ...
 *          client already-paid figure moving 0.00 → 30.00,
 ...
 *        - the row now holds client_paid_amount = 80 (30 authoritative + 50).
 *
 ...
 *
 * Run with: npx tsx test/client-detail-payment-stale-reload-totals-ui.test.ts
 *
 * NOTES:
...
const SUFFIX = `cd-stale-pay-reload-ui-${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    // pulls the authoritative $30 into the open mini-form.
    await page.evaluate(() => {
    ...
    // The "Latest totals loaded" note appears and shows the client figure moving
    // from $0.00 → $30.00 (the displayed already-paid value updated to the latest
    // stored value).
    ...
    await page.waitForFunction(
      () => {
        const el = document.querySelector(
          '[data-testid="refreshed-client-delta"]',
        );
    ...
    const deltaText = await page.evaluate(
      () =>
        document.querySelector('[data-testid="refreshed-client-delta"]')
          ?.textContent || "",
    );
    assert(
      /0\.00/.test(deltaText) && /30\.00/.test(deltaText),
      `The refreshed note shows client already-paid 0.00 → 30.00 (got "${deltaText.trim()}")`,
    );
    ...
    // so a coordinate-based (trusted) click silently lands on the toast instead of
    // the button and never submits. Click the actual button NODE directly (native
    // .click() submits the form regardless of any overlay) to avoid that race.
    await page.waitForFunction(
      () =>
        Array.from(
    ...
    assertEqual(
      ...
      80,
      "The re-submit applied $50 on top of the authoritative $30 → client_paid_amount = 80",
    );
  } catch (error) {
