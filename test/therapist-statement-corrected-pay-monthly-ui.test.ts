/**
 * BROWSER-LEVEL test that the MONTHLY REPORT tab a payout reviewer uses to
 * reconcile a SINGLE MONTH shows the CORRECTED, de-inflated pay after a
 ...
 *     layer that the RUNNING Statement tab shows the corrected $50 after the
 *     cleanup.
 *
 * THIS suite closes the remaining gap: there was no browser-level proof that
 * the "Monthly Report" tab a reviewer actually clicks to reconcile a single
 * month shows the corrected, de-inflated earned-in-month ($50) — and the
 * session's earned cell ($50) — after that cleanup, not the inflated $100.
 *
 ...
 *      affected month, and asserts the "Earned in period" figure
 *      (data-testid `text-earned-month`) AND the session row's earned cell
 *      (`monthly-row-{sessionId}` last cell) both read the corrected single
 ...
 *
 * Run with: npx tsx test/therapist-statement-corrected-pay-monthly-ui.test.ts
 *
 * NOTES:
...
const SUFFIX = `thr-stmt-corrected-monthly-ui-${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    await storage.voidInsuranceStatement(
      statementId,
      billingUserId,
      "duplicate EOB cleanup — monthly-report UI check",
    );
    ...
    // --- 4. Browser flow: load the Monthly Report tab ----------------------
    ...
    // CRITICAL UI money check: the "Earned in period" card shows the CORRECTED
    // single $50 — NOT the inflated $100, and NOT $0.
    ...
    const earnedMonthText = await page.$eval(
      '[data-testid="text-earned-month"]',
    ...
    assertEqual(
      earnedMonthText,
      "$50.00",
      "The 'Earned in period' card shows the corrected single $50 after cleanup (not the inflated $100)",
    );

    // The session's earned cell (the LAST column of its row) reads $50.00. We
    // target the earned cell specifically because the Expected and Collected
    // columns in the same row legitimately show $100.00 — only the EARNED figure
    // distinguishes the corrected ($50) from the inflated ($100) state.
    const rowSelector = `[data-testid="monthly-row-${sessionId}"]`;
    await page.waitForSelector(rowSelector, { timeout: 30_000 });
    const earnedCellText = await page.$eval(
      `${rowSelector} td:last-child`,
      (el: Element) => (el.textContent || "").replace(/\s+/g, " ").trim(),
    );
    assertEqual(
      earnedCellText,
      "$50.00",
      "The session row's earned cell shows the corrected $50.00 (not the inflated $100.00)",
    );
    assert(
      !earnedCellText.includes("$100.00"),
      "The session row's earned cell does NOT show the inflated $100.00",
    );
