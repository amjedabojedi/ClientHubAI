/**
 * BROWSER-LEVEL test that the THERAPIST STATEMENT screen a payout reviewer looks
 * at shows the CORRECTED, de-inflated pay after a duplicate-insurance-payment
 * cleanup — driven through the real /therapist-payments page in a real Chromium.
 *
 ...
 *     negative adjustment so net pay falls from the inflated $100 back to the
 *     correct $50).
 ...
 *
 * THIS suite closes the remaining gap: there was no browser-level proof that the
 * therapist-statement / payout screen a reviewer actually clicks shows the
 * corrected, de-inflated pay ($50) after that cleanup — not the inflated $100.
 *
 ...
 *      therapist, opens the Statement tab, and asserts the displayed
 *      "Total earned" and "Currently owed" both read the corrected single
 *      $50.00 (NOT the inflated $100.00), and the consolidated earning row in
 *      the statement table reads $50.00.
 *
 ...
 *
 * Run with: npx tsx test/therapist-statement-corrected-pay-ui.test.ts
 *
 * NOTES:
...
const SUFFIX = `thr-stmt-corrected-ui-${Date.now()}`;

// A DOM .click() fires the React onClick handler of plain buttons and of cmdk
// CommandItems, sidestepping puppeteer's clickablePoint check (which throws
// "Node is either not clickable or not an Element" for off-screen / mid-animation
// nodes). See .agents/memory/browser-tests-puppeteer.md.
...
async function main() {
  ...
  try {
    ...
    await storage.voidInsuranceStatement(
      statementId,
      billingUserId,
      "duplicate EOB cleanup — therapist-statement UI check",
    );
    ...
    // --- 4. Browser flow: load the therapist statement screen --------------
    ...
    // Tabs appear once a therapist is chosen. Switching to the Statement tab
    // triggers GET /statement server-side, which appends the self-correcting
    // -$50 adjustment and returns the corrected net.
    ...
    // CRITICAL UI money checks: the cards show the CORRECTED single $50 — NOT
    // the inflated $100, and NOT $0.
    ...
    assertEqual(
      balanceText,
      "$50.00",
      "The 'Currently owed' card shows the corrected single $50 after cleanup (not the inflated $100)",
    );

    const earnedText = await page.$eval(
      '[data-testid="text-statement-earned"]',
      (el: Element) => (el.textContent || "").trim(),
    );
    assertEqual(
      earnedText,
      "$50.00",
      "The 'Total earned' card shows the corrected single $50 after cleanup (not the inflated $100)",
    );

    // The consolidated earning row in the statement table reads $50.00 — the
    // earning ($100) and the self-correcting adjustment (-$50) net into one line.
    await page.waitForSelector('[data-testid="statement-row-0"]', {
      timeout: 30_000,
    });
    const rowText = await page.$eval(
      '[data-testid="statement-row-0"]',
      (el: Element) => (el.textContent || "").replace(/\s+/g, " ").trim(),
    );
    assert(
      rowText.includes("$50.00"),
      "The consolidated earning row in the statement table shows the corrected $50.00",
    );
    assert(
      !rowText.includes("$100.00"),
      "The statement row does NOT show the inflated $100.00",
    );
