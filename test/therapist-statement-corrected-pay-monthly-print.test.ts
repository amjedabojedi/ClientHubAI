/**
 * BROWSER-LEVEL test that the PRINTED / "Save as PDF" Monthly Report a payout
 ...
 *     BROWSER layer that the DOWNLOADED CSV (a separate code path from the
 *     on-screen cards) carries the corrected $50.
 *
 * THIS suite closes the remaining gap: a reviewer often reconciles from the
 * PRINTED / "Save as PDF" document. That print document is built by a SEPARATE
 * code path (exportPrint / printHtml in MonthlyReportTab) from both the CSV
 * export and the on-screen cards, so it could drift from what's shown. There
 * was no proof the printed document carries the corrected figure. This asserts
 * the generated print HTML's "Earned in period" card AND the session row's
 * Earned cell both read the corrected $50.00, NOT the inflated $100.00.
 *
 ...
 *      affected month, then clicks the print/PDF export button
 *      (data-testid `button-monthly-print`). Because printHtml opens a popup
 *      window and calls window.print(), the test intercepts window.open and
 *      captures the HTML written to the popup's document, then asserts both the
 *      "Earned in period" card and the session row's Earned cell read the
 *      corrected single $50.00, NOT $100.00.
 *
 ...
 *
 * Run with: npx tsx test/therapist-statement-corrected-pay-monthly-print.test.ts
 *
 * NOTES:
...
const SUFFIX = `thr-stmt-corrected-monthly-print-${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    await storage.voidInsuranceStatement(
      statementId,
      billingUserId,
      "duplicate EOB cleanup — monthly-report print check",
    );
    ...
    // --- 4. Browser flow: load the Monthly Report tab, print/PDF export ----
    ...
    // print assertion below isn't blamed on stale on-screen data.
    ...
    const printSel = '[data-testid="button-monthly-print"]:not([disabled])';
    ...
    assert(
      printHtml.includes("<!doctype html>") && printHtml.includes("Sessions"),
      "Print document is the full report HTML (doctype + Sessions table)",
    );

    // The "Earned in period" summary card is built as
    //   <div class="label">Earned in period</div><div class="value">${money(earnedInMonth)}</div>
    // It MUST read the corrected $50.00 — not the inflated $100.00.
    const cardMatch = /Earned in period<\/div><div class="value">([^<]+)<\/div>/.exec(
      printHtml,
    );
    assert(
      !!cardMatch,
      "Print document contains the 'Earned in period' summary card",
    );
    assertEqual(
      (cardMatch?.[1] || "").trim(),
      "$50.00",
      "Print document 'Earned in period' card reads the corrected $50.00 (not the inflated $100.00)",
    );

    // The session table row carries the Earned column LAST (a .num cell). Find
    // the <tr> data row by our unique client name, then read its final numeric
    // cell. Expected/Collected are legitimately $100.00, so we MUST assert on the
    // last .num cell (Earned), which has to be the corrected $50.00.
    // NOTE: SUFFIX also appears in the therapist name printed in the <h1> title
    // (the pre-table chunk), so we must additionally require the fragment to hold
    // a numeric data cell — otherwise .find() returns that title chunk (which has
    // only card <div>s, no <td class="num">) and the Earned cell reads undefined.
    const rowFragment = printHtml
      .split("<tr>")
      .find((frag) => frag.includes(SUFFIX) && frag.includes('<td class="num'));
    assert(
      !!rowFragment,
      "Print document contains the seeded session's data row",
    );
    const numCells = Array.from(
      (rowFragment || "").matchAll(/<td class="num[^"]*">([^<]*)<\/td>/g),
    ).map((m) => m[1].trim());
    const earnedCell = numCells[numCells.length - 1];
    assertEqual(
      earnedCell,
      "$50.00",
      "Print document session row Earned cell reads the corrected $50.00 (not the inflated $100.00)",
    );
