...
export function AssessmentCompletionDrawer(props: { assignmentId?: number }) {
  const { closeTopDrawer, openDrawer } = useRecordDrawer();
  return (
    <AssessmentCompletionPage
      assignmentId={props.assignmentId}
      onClose={closeTopDrawer}
      onOpenReport={(assignmentId) => {
        // Lateral switch to the report: replace the current drawer (close + open)
        // so we stay at the same depth instead of stacking.
        closeTopDrawer();
        openDrawer({
