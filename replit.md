...
**Live Session Translator:** One-directional real-time translation during Zoom therapy sessions. The therapist speaks in their language; SmartHub transcribes via Whisper, translates via GPT-4o, and pushes the translated text as Zoom closed captions so the client sees subtitles automatically — no extra app needed. The system uses `translation_sessions` and `translation_messages` tables. Audio is captured in 5-second chunks on the client side, sent to `POST /api/sessions/:sessionId/translation/speak`, transcribed + translated, and pushed to Zoom's closed caption API. On session end (`POST /api/sessions/:sessionId/translation/end`), all messages are auto-stitched into the `session_transcripts` table for Smart Fill compatibility. GDPR consent and per-session authorization (assigned therapist, supervisor, or admin) are enforced on all endpoints. The `LiveSessionTranslator` component is rendered alongside `SessionRecorder` in the Session Note dialog. Supports 16 languages including English, Arabic, Spanish, French, Chinese, and more.
