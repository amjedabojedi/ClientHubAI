...
**Session Voice Transcripts (Phase 1):** Therapists can record full therapy sessions (hour-long supported) directly from the Session Note dialog using a chunked client-side recorder. Recording is sliced into 60-second segments; each segment is uploaded as soon as it ends to `POST /api/sessions/:sessionId/transcribe-chunk` (Whisper API), discarded after transcription, and a live preview of recognised text streams back. Reliability & data-safety improvements: chunk text is persisted to the `session_transcripts` row (status `recording`, `chunks` JSONB, `upload_id`) as each chunk arrives — so a server restart mid-recording does not lose already-transcribed text. The client keeps each failed chunk's audio Blob in memory and surfaces a per-chunk **Retry** button that re-uploads the exact failed audio. Closing or refreshing the tab during an active recording shows a `beforeunload` confirm prompt. Finalize is **atomic**: in a single DB transaction the new transcript row is updated to `ready` and any older transcript rows for the same session are deleted — the user is never left with no transcript at all. If the page is refreshed mid-finalize, the saved-transcript view sees the row in `processing` status and polls every 3 s until it transitions to `ready` or `failed`. Whisper is primed with a **language-agnostic clinical-domain prompt** (English-language vocabulary that biases toward therapy terminology without forcing any output language — Whisper auto-detects the spoken language) plus the tail (~200 words) of the previous chunk to maximise accuracy across language switches and chunk boundaries; `temperature: 0` for stability. Sessions can be in any language (English, Arabic, Spanish, French, etc.), mixed (code-switching), or include a live human interpreter. On Stop, `POST /api/sessions/:sessionId/transcribe-finalize` stitches the chunks, runs GPT-4o speaker diarization with **three possible speaker labels — Therapist:/Client:/Interpreter:** (Interpreter is used only when a turn re-states the previous turn's meaning in another language; sessions without an interpreter remain two-speaker), and saves the result to the dedicated `session_transcripts` table (one transcript per session). The transcript is stored as a separate document — it is **not** auto-pasted into the session note; therapists copy/paste the relevant portions themselves. AI consent (GDPR) is enforced before any chunk is transcribed; HIPAA audit logs capture creation, view, and deletion. Pause/resume supported. No audio is persisted server-side. Per-session authorization (assigned therapist, supervisor, or admin only) is enforced on every transcript endpoint. A standalone **View Transcript** action lives in the session card's dropdown menu on the Client Detail page, opening `SessionTranscriptViewer` which fetches the saved transcript and offers Copy / Download .txt / Delete actions.
